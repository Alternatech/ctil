...
export const reqCountPerSocket = new Map();
let terminating = false;

export const serverStoppingHelper = (server: http.Server): void => {
...
export const stoppingFunction = async (): Promise<void> => {
  terminating = true;
  try {
    await Promise.race([
      setTimeout(
...
export const setForceTimeout = (timeout: number): void => {
  forcedStopTimeout = timeout;
};

export const setIdleTimeout = (timeout: number): void => {
